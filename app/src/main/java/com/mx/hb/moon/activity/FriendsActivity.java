...
/**
 * Created by Kiven on 16/7/19.
 */
public class FriendsActivity extends BaseActivity implements View.OnClickListener,SwipeRefreshLayout.OnRefreshListener, SwipeRefreshLayout.OnLoadListener {
    private static SVProgressHUD mSVProgressHUD;
    private TextView titleText;
    private ExpandableListView friendEListView;
    ...
    private void initView() {
        ...
        titleText.setText("好友提成");
        friendEListView = (ExpandableListView) findViewById(R.id.taskEListView);
        ...
        friendEListView.setAdapter(adapter);
    }
    ...
    private void setClickListener() {
        friendEListView.setGroupIndicator(null);// 设置默认图标为不显示状态
        // 设置一级item点击的监听器
        friendEListView.setOnGroupClickListener(new ExpandableListView.OnGroupClickListener() {
            @Override
            public boolean onGroupClick(ExpandableListView parent, View v, int groupPosition, long id) {
                if(friendEListView.isGroupExpanded(groupPosition)){
...
//                    taskEListView.expandGroupWithAnimation(groupPosition);
        ...
        // 设置二级item点击的监听器
        friendEListView.setOnChildClickListener(new ExpandableListView.OnChildClickListener() {
            @Override
    ...
    private void requestList() {
        ...
        asyncHttpClientUtil.doPost(Constants.FIND_FRIENDS_RECORD, params, new AsyncHttpResponseHandler() {
            @Override
            public void onStart() {
                super.onStart();
            }
...
//            Log.e("requestList",jsonObject.toString());
            ...
            if(!TextUtils.equals("10",code)){
                ...
                for(int i = 0;i < jsonArray.size();i++){
                    ...
                    map.put("state",date.substring(date.length()-9,date.length()));
                    parents.add(map);
                }
                ...
                for(int i = 0;i < jsonArray.size();i++){
                    ...
                    map2.put("time", date.substring(0,date.length() - 9));
                    ls.add(map2);
                    childs.add(ls);
                }
                ...
                runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        adapter.notifyDataSetChanged();
                        if (mPageIndex == 1) {
                            friendEListView.setSelectionAfterHeaderView();
                        } else {
                            friendEListView.setSelection(mPageIndex * 10);
                        }
                    }
                });
            }
