...
import com.mx.hb.moon.activity.MainActivity;
import com.mx.hb.moon.base.BannerHandler;
 ...
 */
@SuppressLint("NewApi")
public class BannerImageView extends Fragment implements OnPageChangeListener {
	...
	private BannerViewPager parentViewPager;
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View view = LayoutInflater.from(getActivity()).inflate(R.layout.view_banner_content, null);
	...
	private void startBannerHandler() {
		handler = new BannerHandler(getActivity()) {
			@Override
	...
	public void setData(List<ImageView> views, List<ADInfo> list, ImageCycleViewListener listener, int showPosition) {
		...
		//如果图片数量大于1张才开始设置指示的小图标
		if(indicators.length > 1){
			for (int i = 0; i < indicators.length; i++) {
				View view = LayoutInflater.from(getActivity()).inflate(R.layout.view_banner_indicator, null);
		...
		if(indicators.length > 1){// 默认指向第一项，下方viewPager.setCurrentItem将触发重新计算指示器指向
			setScrollable(true);
			setIndicator(0);
		} else {
			setScrollable(false);
...
//		indicatorLayout.setLayoutParams(params);
	 ...
	 */
	public void disableParentViewPagerTouchEvent(BannerViewPager parentViewPager) {
		if (parentViewPager != null)
